import re
import numpy as np
from methods import load_imdb_data, extract_features, vectorizer, calculate_metrics
...
# listes accuracy precision recall F1 training dedomena RF SKLEARN
accuracy_scores_train_rf = []
precision_scores_train_rf = []
recall_scores_train_rf = []
f1_scores_train_rf = []

# listes accuracy precision recall F1 test dedomena RF SKLEARN
accuracy_scores_test_rf = []
precision_scores_test_rf = []
recall_scores_test_rf = []
f1_scores_test_rf = []

# listes accuracy precision recall F1 training dedomena NAIVE BAYES BERNOULLI SKLEARN
accuracy_scores_train_nb = []
precision_scores_train_nb = []
recall_scores_train_nb = []
f1_scores_train_nb = []

# listes accuracy precision recall F1 test dedomena NAIVE BAYES BERNOULLI SKLEARN
...
for size in training_sizes:
    ...
    # EKPAIDEYSH RANDOM FOREST SKLEARN
    trainRFID3 = RandomForestClassifier(criterion='entropy')
    trainRFID3.fit(np.array(X), np.array(Y))  # algorithmos random forest

    train_resultsRFID3 = trainRFID3.predict(
        X)  # apotelesmata bash tou random forest sklearn gia ta training data
    test_resultsRFID3 = trainRFID3.predict(
        X_test)  # apotelesmata bash tou random forest sklearn me kapoia test data
    ...
    # EKPAIDEYSH NAIVE BAYES SKLEARN
    trainNB = BernoulliNB()
    trainNB.fit(np.array(X), np.array(Y))

    train_resultsNB = trainNB.predict(X)  # apotelesmata bash tou naive bayes bernoulli sklearn gia training data
    test_resultsNB = trainNB.predict(X_test)  # apotelesmata bash tou naive bayes bernoulli sklearn gia kapoia test data
...
# ektypwnoume pinakes Random Forest & Naive Bayes me pososta accuracy, precision, recall & F1 gia kathe megethos training data
