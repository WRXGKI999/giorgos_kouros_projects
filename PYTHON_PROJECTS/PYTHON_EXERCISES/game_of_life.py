...
def get_iterator(board):
    ...
    """
    """GRAPSTE TON KWDIKA SAS EDW."""
    return sorted(iter(board.items()))
...
def place_blinker(board, p = (0,0)):
    ...
    """
    """GRAPSTE TON KWDIKA SAS EDW."""
    for x in range(p[0],p[0]+3):
        set_alive(board,(x,p[1]),True) 
...
def place_glider(board, p = (0,0)):
    ...
    """
    """GRAPSTE TON KWDIKA SAS EDW."""
    for x in range(p[1],p[1]+3):
        if x==p[1]:
            set_alive(board,(p[0]+1,x),True)
        elif x==p[1]+1:
            set_alive(board,(p[0]+2,x),True)
        else:
            for y in range(p[0],p[0]+3):
                set_alive(board,(y,x),True)
...
def tick(board):
    ...
    for x in range(rng):
        for y in range(rng):
            ...
            if b[x,y]:
                if n<=1 or n>=4:
                    set_alive(board,(x,y),False)
            else:
                if n==3:set_alive(board,(x,y),True)
            n=0
